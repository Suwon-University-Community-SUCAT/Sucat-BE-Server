...
public class BoardService {
    ...
    /* 게시글 좋아요 메서드 */
    @Transactional
    public boolean likeBoard(Long boardId, HttpServletRequest request) {
        User user = userService.getUserInfo(request);
        Board board = findBoardById(boardId);

        if (board.getLikedUsers().contains(user)) {
            board.removeLike(user);
            log.info("User {} removed like from board {}", user.getId(), boardId);
            return false;
        } else {
            board.addLike(user);
            log.info("User {} liked board {}", user.getId(), boardId);
            return true;
        }
    }
    @Transactional
    public boolean isLikedByUser(Long boardId, HttpServletRequest request) {
        User user = userService.getUserInfo(request);
        Board board = findBoardById(boardId);
        return board.getLikedUsers().contains(user);
    }
